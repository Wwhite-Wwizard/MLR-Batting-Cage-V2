...
function botPitch(ranges) {
  ...
  // 70% chance: add a random jitter of up to ±500 to stay unpredictable
  if (Math.random() < 0.7) {
    base = Math.floor(Math.random() * 1000) + 1;
  }
...
function scoreboard(g) {
  ...
  const results  = g.history.map(h => getResult(h.diff, g.ranges).label);
  const PA       = results.length;
  const hits     = results.filter(r => ['HR','3B','2B','1B'].includes(r)).length;
  const BB       = results.filter(r => r === 'BB').length;
  const AB       = results.filter(r => r !== 'BB').length;
  const totalBases = results.reduce((sum, r) => {
    if (r === 'HR') return sum + 4;
    if (r === '3B') return sum + 3;
    if (r === '2B') return sum + 2;
    if (r === '1B') return sum + 1;
    return sum;
  }, 0);

  const BA  = AB  > 0 ? (hits / AB).toFixed(3).replace(/^0/, '')         : '.000';
  const OBP = PA  > 0 ? ((hits + BB) / PA).toFixed(3).replace(/^0/, '')  : '.000';
  const SLG = AB  > 0 ? (totalBases / AB).toFixed(3).replace(/^0/, '')   : '.000';
  const OPS = (parseFloat(OBP) + parseFloat(SLG)).toFixed(3).replace(/^0/, '');

  const avgDiff  = (g.history.reduce((sum, h) => sum + h.diff, 0) / PA).toFixed(1);
  const totalPts = g.history.reduce((sum, h) => sum + getResult(h.diff, g.ranges).points, 0);

  return (
    `\n\n📋 **Scoreboard**\n` +
    rows.join('\n') +
    `\n\nAverage diff: **${avgDiff}** · Total points: **${totalPts}**` +
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'startgame') {
    ...
    const pitcherIsBot = pitcherUser.id === client.user.id || pitcherUser.bot === true;
    const batterIsBot  = batterUser.id  === client.user.id || batterUser.bot  === true;

    console.log(`[startgame] pitcher: ${pitcherUser.id} (bot=${pitcherUser.bot}) | batter: ${batterUser.id} (bot=${batterUser.bot}) | client: ${client.user.id}`);
  ...
  if (commandName === 'pitch') {
    ...
    // If the batter is the bot, swing automatically
    if (game.batterIsBot) {
      const botSwingValue = botSwing(game.ranges);
      await interaction.followUp({
        content:
          `⚾ **Turn ${game.turn}/${TOTAL_TURNS}** — 🟡 Pitcher has thrown a pitch!\n` +
          `🤖 Bot swings **${botSwingValue}**!`,
      });
      await resolveTurn(interaction, guildId, botSwingValue);
    } else {
      await interaction.followUp({
        content:
          `⚾ **Turn ${game.turn}/${TOTAL_TURNS}** — 🟡 Pitcher has thrown a pitch!\n` +
