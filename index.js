...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (interaction.customId === 'select_pitcher_type') {
      if (userId !== game.starterId) {
        await interaction.reply({ content: `⚠️ Only the person who started the game can make setup selections.`, ephemeral: true });
        return;
      }
    ...
    if (interaction.customId === 'select_hand_bonus') {
      ...
      game.handBonus = val;
      await interaction.update({
        content: `🟡 Hand bonus set to **${val}**. Now select the pitcher's handedness:`,
        components: [handednessMenu('select_pitcher_hand', 'Select pitcher handedness…')],
      });
      return;
    }
    ...
    if (interaction.customId === 'select_pitcher_hand') {
      ...
      await interaction.update({
        content: `🟡 Pitcher is **${handLabel}**. Now select the batter type:`,
    ...
    if (interaction.customId === 'select_batter_type') {
      ...
      game.batterType = val;
      await interaction.update({
        content: `🔵 Batter type set to **${val}**. Now select the batter's handedness:`,
        components: [handednessMenu('select_batter_hand', 'Select batter handedness…')],
      });
      return;
    }
    ...
    if (interaction.customId === 'select_batter_hand') {
      ...
      game.batterHand = val;
      await interaction.update({
        content: `🔵 Batter handedness set. Finalising setup…`,
        components: [],
      });
  ...
  if (commandName === 'startgame') {
    ...
    // Check if either is the bot itself
    ...
    await interaction.reply({
      content:
        ...
        `<@${userId}> — select the pitcher type to begin setup:`,
      components: [pitcherTypeMenu()],
    });
    return;
  }
